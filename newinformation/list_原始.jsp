        ...
        <form method="post" id="mainwrap">
            ...
            <div class="bodywrap">
                <table class="wp100 tc cellbor" id="section">
                    ...
                    <tbody>
                        <%
                            RssListView list = null;
                            if (cookie.Get("powergroupid").equals("5")) {
                                list = new RssListView(pageContext, "lzmessage_newsuser");
                            } else {
                                list = new RssListView(pageContext, "newinformation");
                            }
                            list.request();
                            int a = 1;
                            list.pagesize = 30;
                            list.select().where("title like '%" + URLDecoder.decode(list.get("title"), "utf-8") + "%' and realname like '%" + URLDecoder.decode(list.get("realname"), "utf-8") + "%' and myid like '%"+cookie.Get("myid")+"%'").get_page_desc("id");
        ...
        <script>
         ...
         $(function(){
          ...
          $(function(){
                ﻿$(".edit").click(function(){
                    let id = $(this).attr("id");
                popuplayer.display("/newinformation/edit.jsp?id=" + id + "&TB_iframe=true", '编辑', {width: 790, height: 580});
                })
            });    
