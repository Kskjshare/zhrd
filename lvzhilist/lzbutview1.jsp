...
//                                    + "%' and realname like '%" + URLDecoder.decode(list.get("realname"), "utf-8") + "%'" + " and classify=1" + " and userid="+req.get("id")).get_page_desc("id");

      
//                            list.select().where("title like '%" + URLDecoder.decode(list.get("title"), "utf-8") + "%' and name like '%" + URLDecoder.decode(list.get("name"), "utf-8") + "%' and department like '%" + URLDecoder.decode(list.get("department"), "utf-8") 
//                                    + "%' and realname like '%" + URLDecoder.decode(list.get("realname"), "utf-8") + "%'" + " and attendancetype=2" + " and userid="+req.get("id") + " and classify=1").get_page_desc("id");
                            list.select().where("classify=1" + " and auditState=2 and userid="+ req.get("id")).get_page_desc("id");
                        ...
                        <tr ondblclick="ck_dbcglclick();" idd="<% out.print(list.get("id")); %>" style="cursor:pointer;">
                            ...
                            <td><% out.print("出席人大会"); %></td>
                            <!--<td><% out.print(list.get("classify")); %></td>-->
        ...
        <script src="controller.js"></script>
         ...
         $(function(){
                ﻿$(".view").click(function(){
                    let id = $(this).attr("id");
                     popuplayer.display("/lvzhilist/lzbutview.jsp?id=" + id + "&TB_iframe=true", '查看内容', {width: 500, height: 550});
                })
            });
