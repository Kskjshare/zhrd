...
//      }
      ...
      while ( list.for_in_rows() ) {
        ...
        while ( list3.for_in_rows() ) { //计算建议议案
...
//              totalsuggests ++ ;
          ...
          while (secondlist.for_in_rows()) {
            ...
            list4.query("SELECT classify,COUNT(*) AS num FROM activities_userlist_list_view WHERE userid=" + list.get("myid") + " and attendancetype=2 and auditState=2 GROUP BY classify");    
            String ddd = "";
            while (list4.for_in_rows()) {                     
                ...
                //这里需要特殊处理  //这个地方以后还需要处理，因为如果用户单独上传视察调研和执法检查的话。20200628
                if ( z == 6  ) {
                    //处理视察调研执法检查
...
//            }
    ...
    String myranklistScore = myranklist.get("totalScore")  ;
    RssList usrlist = new RssList(pageContext, "user");
    usrlist.request();
    usrlist.select().where("myid="+UserList.MyID(request)).get_first_rows();
    if ( !usrlist.get("isdelegate").equals("1") ) {
        myranklistScore = "0" ;
    }
