    ...
    <body>
                ...
                <table class="wp100 tc cellbor" id="section">
                    ...
                    <tbody>
                        <%
                            ...
                            list.pagesize = 30;
                            int isTermination = 0 ;
                            String stateStr = "立法执行初审";
                            list.select().where("title like '%" + URLDecoder.decode(list.get("title"), "utf-8") + "%' and state <> 100").get_page_desc("id");
                            while (list.for_in_rows()) {
                                isTermination = 0 ;
                                

                                if ( list.get("state").equals("0") )  {
                                   out.print(list.get("初审"));  
                                   isTermination = 1 ;
                                }
                                ...
                                else if ( list.get("state").equals("99") )  {
                                   stateStr = "终止审议" ;                                  
                        ...
                        <tr ondblclick="ck_dbablclick();" idd="<% out.print(list.get("id")); %>" style="cursor:pointer;">
                            ...
                            if ( isTermination == 1 ) {
                            %>   
                            | <span class="ys terminate" id="<% out.print(list.get("id")); %>" style="color:red;font-weight: bold" >终止审议</span> 
                            <% 
                            }
                            %>
        ...
        <script>
            ...
            ﻿$(".terminate").click(function(){
                let id = $(this).attr("id");
             popuplayer.display("/legislative/legistationvoting/auditview.jsp?id=" + id + "&TB_iframe=true", '终止审议', {width: 860, height: 460});
