   ...
   RssList legislation_classify = new RssList(pageContext, "legislation_classify");
    ...
    if (!entity.get("action").isEmpty()) {
        switch (entity.get("action")) {
            ...
            case "audit":
                if ( entity.get( "auditresult").equals("1") ) {
                   // entity.keyvalue("state","1");
                out.print("<script>alert('auditresult1')</script>");

                }
                else if ( entity.get( "auditresult").equals("2") ) {
                 ...
                 // entity.update().where("id=?", req.get("id")).submit();
                   ...
                   out.print("<script>alert('auditresult2')</script>");
                }
        ...
        <form method="post" class="popupwrap">           
            <div>
                <table class="wp100 cellbor">
                   <tr><td style="font-size:20px;font-family: 微软雅黑" colspan="7" id="tabheader">法制委审议</td></tr>
                    <tr>
            ...
            <div class="footer">
                ...
                <input type="hidden" name="action" value="<% out.print("audit");%>" />
                <button style="float:left;" type="submit"  ><% out.print("提请会议表决");%></button>
                <button type="submit"><% out.print("提请二审");%></button>
                <input name="auditresult" type="hidden">
            </div>
        </form>
        <!--<script src="/data/suggest.js" ty //pe="text/javascript"></script>-->
        <%@include  file="/inc/js.html" %>
        <script>
            $(".footer>button").click(function () {
              
                $('.footer button:first').click(function () {
                    $('[name=auditresult]').val(1);
                });
                $('.footer button:last').click(function () {
                    $('[name=auditresult]').val(2);
                });
