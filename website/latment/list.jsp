                ...
                <table class="wp100 tc cellbor" id="section">
                    ...
                    <tbody>
                        <%
                            ...
                            int a = 1;
                            list.pagesize = 30;
                            list.select().where("title like '%" + URLDecoder.decode(list.get("title"), "utf-8") + "%' and classifyid=8").get_page_desc("id");
...
//                                list.keyvalue("userrolecompany", user_delegation.get("company"));
        ...
        <script>
              $(function(){
                ﻿$(".view").click(function(){
                    let id = $(this).attr("id");
                popuplayer.display("/website/butview.jsp?id=" + id + "&TB_iframe=true", '查看内容', {width: 830, height: 240});
                })
            });
             $(function(){
            ﻿$(".delete").click(function(){
             ...
             popuplayer.display("/website/latment/delete.jsp?relationid=" + id + "&TB_iframe=true", '删除', {width: 300, height: 80});
//             popuplayer.display("/website/delete_1.jsp?id=" + id + "&TB_iframe=true", '删除', {width: 300, height: 80});
             ...
             $(function(){
                ﻿$(".edit").click(function(){
                    let id = $(this).attr("id");
                popuplayer.display("/website/latment/edit.jsp?id=" + id + "&TB_iframe=true", '编辑', {width: 830, height: 380});
                })
            });
            ...
            ﻿$(".toutiao").click(function(){
                let id = $(this).attr("id");
             //popuplayer.display("/supervision/zhifajiancha/delete.jsp?id=" + id + "&TB_iframe=true", '删除', {width: 300, height: 80});
             popuplayer.display("/website/latment/headline.jsp?id=" + id + "&TB_iframe=true", '设置头条新闻', {width: 400, height: 140});
            ...
            ﻿$(".horselight").click(function(){
                let id = $(this).attr("id");
             //popuplayer.display("/supervision/zhifajiancha/delete.jsp?id=" + id + "&TB_iframe=true", '删除', {width: 300, height: 80});
             popuplayer.display("/website/latment/horselight.jsp?id=" + id + "&TB_iframe=true", '设置图片新闻', {width: 360, height: 120});
