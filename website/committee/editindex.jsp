...
<%
    ...
    entity.request();
    entity2.request();
    System.out.println("aciton111=="+entity.get("action"));
     System.out.println("aciton1222=="+!entity.get("action").isEmpty());
...
//                } else {
                if (entity3.select().where("account='" + entity.get("account") + "' and myid <> " + entity.get("relationid") + "").get_first_rows()) {
                    ...
                    entity.keyvalue("account", entity.get("account"));
                    entity.keyvalue("pwd", Encrypt.Md532(entity.get("account") + entity.get("passwordNO")));
                    System.out.println("avatar333=="+entity.get("avatar"));
                    entity.keyvalue("avatar", entity.get("avatar"));//added by jackie
            ...
            <div>
                <table class="wp100 formor">
                    ...
                    <tr>
                        ...
                        <td><select name="mission">
                                <%
                                    ...
                                    company.select().where("state like '%4%' and level in (0,1)  order by daibiaotuanCode").query();
                    ...
                    <tr>
                        ...
                        <td><select class="w260" name="clan" dict-select="partytypeclassify" def="<% entity2.write("clan"); %>"></select></td>
                    ...
                    <tr>
                        ...
                        <td><input type="text" maxlength="80" class="w254" name="telphone" value="<% entity2.write("telphone"); %>" /><em>*</em></td>
                    </tr>
                    <tr>
...
//                                $("input[name='circleslist']").val($(".circles option:selected").val() + ",")
                            ...
                            function PreviewImage(fileObj, imgPreviewId, divPreviewId) {
                                ...
                                //alert("allow22::"+extention);//jackie debug
                                if (allowExtention.indexOf(extention) > -1) {
                                   // alert("allow22::"+extention);//jackie debug
                                    if (fileObj.files) {//HTML5ʵ��Ԥ��������chrome�����7+��
                                     //   alert("allow33==::");//jackie debug
                                        if (window.FileReader) {                                           
                                            var reader = new FileReader();
                                            reader.onload = function (e) {
                                                document.getElementById(imgPreviewId).setAttribute("src", e.target.result);
                                                // alert("allow44::"+$("#imgPreviewId").attr("src");//jackie debug
                                            }
                                            //alert("allow44::"）;//jackie debug
...
//                                                fileObj.blur(); //������document.selection.createRange().text��ie9��
                            ...
                            $("#submit").click(function () {
                                $("#abc").ajaxSubmit({
                                    ...
                                    dataType: "json",
                                    async: false,
                                    //success: function (data) {//commented by jackie
                                    success: function (data) {
                                        console.log(data)
                                       // $("#minhead>img").attr("src", "/upfile/" + data.url);//此id在此页面不存在commented by jackie
                                        $("#imgHeadPhoto>img").attr("src", "/upfile/" + data.url);//给此id的src属性赋值也不成功，data.url看来是空串
                                        
                                       // $("#avatar").val(data.url);//commented by jackie
                                        //$("#avatar").val("wwwx");//jackie debug
                                        //alert("上传成功=="+data.url);//jackie debug
                                       // $("#upfiletype").text("(成功上传)")//commented by jackie
                                       // $("#upfiletype").text(data.url);////jackie debug
                                     //  alert("success::true::");
                                    //    alert("data.error::$=="+data.error);
                                    //  alert("：：error::是否为空：："+data.error=="");
                                       if(data.error==""){//added by jackie
                                           $("#upfiletype").text("(成功上传)");
                                           $("#avatar").val(data.url);
                                       }else{
                                           $("#upfiletype").text("(上传失败)"+data.error);
                                       }
                                    }});
                                return false;
                            })
                            $('#pbut').click(function () {
                                $("#submit").click();
                               // alert("avatarattr$$==::"+$("#avatar").attr("value"));
                            })
