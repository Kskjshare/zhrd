    ...
    <body>
        <form method="post"  class="labelright nomargin" id="uesrupfile">           
            <div class="mainwrapassist notitletoolbar">
                <input type="file" name="img" id="file" multiple><a href="/upfile/moban/DWLXR.xlsx">下载模板</a>
                <table class="wp100 cellbor">
                    <tr><td>代表证号</td><td>姓名</td><td>性别</td><td>民族</td><td>出生年月</td><td>身份证号</td><td>单位职务</td><td>详细地址</td><td>手机号码</td><td>代表阶层</td><td>备注</td></tr>  
            ...
            $("#button").click(function () {
                ...
                set = setInterval(function () {
                    ...
                    if (state == "5") {
                        ...
                        $("#abc>h1").text(ts);
                        if (err != "no") {
                            $("#abc tbody").append("<tr><td>姓名</td><td>电话号码</td><td>单位名称</td><tr>")
                            $.each(err, function (k, v) {
                                $("#abc tbody").append("<tr><td>" + v.realname + "</td><td>" + v.tel + "</td><td>" + v.allname + "</td></tr>")
                ...
                $("#uesrupfile").ajaxSubmit({
                    ...
                    success: function (json) {
                        $.get("/company/companyuser/excel.jsp", {'n': json.url}, function (e) {
                            var a = JSON.parse(e);
