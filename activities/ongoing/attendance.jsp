...
//        }
         ...
         //if (!entity2.get("userid").isEmpty()) 
        {
            ...
            user.keyvalue("attshijian", systemTime);
   ...
   //added by ding for check status of attendance
   user.request();
   if ( cookie.Get("powergroupid").equals("5") ) {
    user.select().where("activitiesid="+ entity.get("id") + " and userid="+ UserList.MyID(request) ).get_first_rows();  
   }
   else {
       user.select().where("activitiesid=?", entity.get("id")  ).get_first_rows();  
   }   

//   user.select().where("activitiesid="+ entity.get("id") + " and userid="+ UserList.MyID(request) ).get_first_rows();   
   //user.select().where("activitiesid='" + entity.get("id") + "' and userid='" + UserList.MyID(request) + "'").get_first_rows();
  //user.select().where("id=?", entity.get("id") ).get_first_rows();                           
%>
<!DOCTYPE html>
<html>
    <head>
...
<!--                            <input type="text" class="w200 Wdate" name="beginshijian"  rssdate="<% out.print(entity.get("beginshijian")); %>,yyyy-MM-dd"   onFocus="WdatePicker({lang: 'zh-cn'})" readonly="readonly">-->
                    ...
                    <tr>
                        ...
                        <td rssdate="<% out.print(entity.get("finishshijian")); %>,yyyy-MM-dd" >
<!--                            <input type="text" class="w200 Wdate" name="finishshijian"  rssdate="<% out.print(entity.get("finishshijian")); %>,yyyy-MM-dd"   onFocus="WdatePicker({lang: 'zh-cn'})" readonly="readonly">-->
                        </td>
                    </tr>
                    <tr>
                     ...
                     <tr>
                        ...
                        <td><% 
                            if ( user.get("attendancetype").equals("1")){
                                out.print("<em style='color:#00cc33'>未考勤</em>");
                            }
                            else {
                                 out.print("<em style='color:#999900'>已考勤</em>");
                            }
                        %></td>
                    </tr>
                    <!--
                    <tr>
                        ...
                        <td id="unseluserlist">
                            ...
                            <%
                                ...
                                if (cookie.Get("powergroupid").equals("5")) {
                                    entity4.select().where("activitiesid=" + entity.get("id") + " and attendancetype=1 and jointype=2 and userid=" + UserList.MyID(request)).query();
                                } else {
