    ...
    if (!entity.get("action").isEmpty()) {
        switch (entity.get("action")) {
            ...
            case "update":
                ...
                String str2 = entity2.get("userid");
                String[] arry2 = str2.split(",");
                for (int i = 0; i < arry2.length; i++) {
                    if (!arry2[i].isEmpty()) {

                        user.timestamp();
                        user.keyvalue("activitiesid", entity2.get("id"));
                        user.keyvalue("userid", arry2[i]);
                        user.keyvalue("myid", entity2.get("myid"));
                        user.append().submit();

                        senduser.pagesize = 10000000;
                        senduser.select().where("myid=" + arry2[i] + " and state=1").get_page_desc("id");
                        while (senduser.for_in_rows()) {
                            if (!(senduser.get("deviceid").isEmpty())) {
                                jpushClient.sendPush(PushPayload.newBuilder().setPlatform(Platform.android()).setAudience(Audience.registrationId(senduser.get("deviceid"))).setNotification(Notification.alert("您有一条新的活动！")).setOptions(Options.newBuilder().setApnsProduction(false).setSendno(ServiceHelper.generateSendno()).setTimeToLive(86400).build()).setMessage(Message.content("您收到一条活动")).build());
                            }
                        }
                    }
                }
            ...
            <div>
                <table class="wp100 cellbor">
                    ...
                    <tr>
                        ...
                        <td >
                           <input id="ziyuan" style="width:55px;height:16px;" name="leixing"  type="radio" value="1">自愿报名
                           <input id="qiangzhi" style="width:55px;height:16px;"  name="leixing"  type="radio" value="2">强制报名
                        </td>
                    </tr>
                    -->
                    
                    <tr>
                        <td class="dce" style="width:110px;">标题</td>
                        <td><input type="text" maxlength="80" class="w250" name="title" value="<% entity.write("title"); %>" /></td>
                    </tr>
                    <tr>
                        <td class="dce" style="width:110px;">活动类型</td>
                        <td><select class="w250" name="classify" dict-select="activitiestypeclassify1" def="<% entity.write("classify"); %>"></select></td>
                    </tr>
<!--                    <tr>
                        <td class="dce" style="width:110px;">组织单位</td>
                        <td><input type="text" maxlength="80" class="w250" name="department" value="<% user2.write("realname"); %>" /></td>
                    </tr>-->
                    <tr>
                        <td class="dce "style="width:110px;" >组织单位</td>
                        <td   id="userrolesel">
                            <input class="w250" type="text" maxlength="80"  readonly="readonly" value="<% entity.write("department"); %>"/>
                            <input type="hidden" name="department" id="userroleid" value="<% entity.write("department"); %>"/>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td class="dce" style="width:110px;">邀请参与代表</td>
                    ...
                    <tr>
                        <td class="dce  left" style="width:110px;"><span>活动内容</span></td>
                    ...
                    <tr style="display: none;"><td>审查人</td>
                        ...
                        %>
                        <td colspan="5">
                            <input type="text" value="<% user1.write("linkmanmission"); %>" >
                        </td>
                    </tr>
                    <tr class="thismyid">
                        <td class="tr">作者ID：</td>
...
//                                    console.log(dict);
                                ...
                                if (coo == "22") {
                                    RssWin.open("/activities/selectuser_2.jsp?mission=<%=cookie.Get("myid")%>", 500, 550);
                                } else {
                                    RssWin.open("/activities/selectuser_2.jsp?mission=<%=user1.get("mission")%>", 500, 550);
                                }
                            });
                            $("#seluserlist i").off("click").click(function () {
                                $(this).parent().remove();
...
//                                }
                            ...
                            $("#userrolesel>input").click(function () {
                                ...
                                RssWin.open("./selectuserrole_1.jsp", 700, 650);
                            });
        </script>
    </body>
</html>
