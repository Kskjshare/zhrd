...
//                                            URLDecoder.decode(req.get("meetingnum"), "utf-8") 
                            ...
                            while (list.for_in_rows()) {
                                ...
                                //读取专班单位
                                String company = list.get("company"); 
                                boolean istransferd = false;
                                ...
                                if ( !company.isEmpty() ) {
                                    ...
                                    for ( int i = 0 ; i < companies.length; i ++ ) {
                                        if ( companies [ i] .equals( UserList.MyID(request) ) ) {
                                            istransferd = true ;
                                            break;
                                        }
                                ...
                                //判断是否已经回复
                                entityReply.select().where("relationid=" + list.get("id") + " and replyid=" + UserList.MyID(request) ).get_first_rows();
                                if ( !entityReply.get("replyinfo").isEmpty() ) {
                                    showReply = false ;
                                }
                                entityReply.remove();
...
-->                            <td>
                                 ...
                                 | <span class="ys shenpi" style="font-weight:bold;" id="<% out.print(list.get("id")); %>">审查</span> 
                                 ...
                                 if ( showReply ){ 
                                 %>   
                                  | <span class="ys reply" style="font-weight:bold;" id="<% out.print(list.get("id")); %>">回复</span> 
                                 <% 
                                 }
                                 %>  
        ...
        <script>
            if(<% out.print(UserList.MyID(request));%> == 1024){
             ...
             $(function(){
                ﻿$(".chakan").click(function(){
                    let id = $(this).attr("id");
                    popuplayer.display("/judicsuper/jfxwjba/butview.jsp?id=" + id + "&TB_iframe=true", '查看', {width: 900, height: 330});
                })
            });
             $(function(){
            ...
            $(function(){
                ﻿$(".reply").click(function(){
                    let id = $(this).attr("id");
                    popuplayer.display("/judicsuper/jfxwjba/tranreply.jsp?id=" + id , '回复', {width: 800, height: 360});
                })
            });
