...
//                            list.select().where("typeid=1").get_page_desc("id"); 
                            list.select().where("title like '%" + URLDecoder.decode(list.get("title"), "utf-8") + "%'" + " and typeid=3").get_page_desc("id"); 
                            //list.select().where("title like '%" + URLDecoder.decode(list.get("title"), "utf-8") + "%'").get_page_desc("id"); 
//                            list.select().where("typeid=3").get_page_desc("id"); 
        ...
        <script>
             $(function(){
                ﻿$(".chakan").click(function(){
                    let id = $(this).attr("id");
                popuplayer.display("/judicsuper/collhaquin/butview.jsp?id=" + id , '查看内容', {width: 800, height: 260});
                })
            });
            $(function(){
                ﻿$(".bianji").click(function(){
                    let id = $(this).attr("id");
                popuplayer.display("/judicsuper/collhaquin/edit.jsp?id=" + id , '编辑', {width: 800, height: 500});
                })
            });
            $(function(){
                ﻿$(".viewreply").click(function(){
                    let id = $(this).attr("id");
                popuplayer.display("/judicsuper/replylist.jsp?id=" + id +"&typeid=3", '查看回复', {width: 800, height: 380});
                })
            });
            ...
            $(function(){
                ﻿$(".reply").click(function(){
                    let id = $(this).attr("id");
                popuplayer.display("/judicsuper/collhaquin/replyview.jsp?id=" + id , '提供线索', {width: 800, height: 380});
                })
            });
             ...
             $(function(){
                ﻿$(".shanchu").click(function(){
                    let id = $(this).attr("id");
                popuplayer.display("/judicsuper/collhaquin/delete_1.jsp?id=" + id , '删除', {width: 300, height:80});
                })
            });
        </script>
