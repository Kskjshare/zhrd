...
//                            list.select().where("typeid=1").get_page_desc("id"); 
                            list.select().where("title like '%" + URLDecoder.decode(list.get("title"), "utf-8") + "%'" + " and typeid=4").get_page_desc("id"); 
                            //list.select().where("title like '%" + URLDecoder.decode(list.get("title"), "utf-8") + "%'").get_page_desc("id"); 
//                            list.select().where("typeid=4").get_page_desc("id"); 
...
//                popuplayer.display("/judicsuper/collnarcl/butview_1.jsp?id=" + id , '查看回复', {width: 800, height: 380});
                 popuplayer.display("/judicsuper/replylist.jsp?id=" + id +"&typeid=4", '查看回复', {width: 800, height: 380});
                })
            });
