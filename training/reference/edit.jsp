...
//    Collection<String> arry = new ArrayList<String>();
    ...
    map.put("key", "10");
    RssListView entity = new RssListView(pageContext, "reference");
    RssListView entity3 = new RssListView(pageContext, "reference");
    RssList entity2 = new RssList(pageContext, "reference");
    RssList read = new RssList(pageContext, "referenceobj");
    entity.request();
    entity2.request();
//    entity.request();
    if (!entity.get("action").isEmpty()) {
        switch (entity.get("action")) {
            case "append":
                ...
                if (!entity.get("objid").equals(",")) {
                    ...
                    for (int i = 0; i < bb.length; i++) {
                        if (!bb[i].isEmpty()) {
                            read.keyvalue("referenceid", entity2.autoid);
                            ...
                            while (senduser.for_in_rows()) {
                                ...
                                jpushClient.sendPush(PushPayload.newBuilder().setPlatform(Platform.android()).setAudience(Audience.registrationId(senduser.get("deviceid"))).setNotification(Notification.android("您有一条新的履职参考！","履职参考",map)).setOptions(Options.newBuilder().setApnsProduction(false).setSendno(ServiceHelper.generateSendno()).setTimeToLive(86400).build()).setMessage(Message.content("您收到一条履职参考")).build());
                                }
                            }
                        }
                    }
            ...
            case "update":
                ...
                read.delete().where("referenceid=?", entity.get("id")).submit();
                if (!entity.get("objid").equals(",")) {
                    String[] bb = entity.get("objid").split(",");
                    for (int i = 0; i < bb.length; i++) {
                        if (!bb[i].isEmpty()) {
                            read.keyvalue("referenceid", entity.get("id"));
                            read.keyvalue("objid", bb[i]);
                            read.append().submit();
                            senduser.pagesize = 10000000;
                            senduser.select().where("myid=" + bb[i] + " and state=1").get_page_desc("id");
