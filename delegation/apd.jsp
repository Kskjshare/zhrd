    ...
    <body>
        <form method="post"  class="labelright nomargin" id="uesrupfile">           
            <div class="mainwrapassist notitletoolbar">
                <input type="file" name="img" id="file" multiple><a href="./DaiBTGL.xlsx">下载模板</a>
                <table class="wp100 cellbor">
                    <tr><td>代表团名称</td><td>代表团编码</td><td>联系地址</td><td>联系人</td><td>联系方式</td><td>邮政编码</td><td>电子邮箱</td></tr>  
            ...
            $("#button").click(function () {
                ...
                set = setInterval(function () {
                    ...
                    if (state == "4") {
                        ...
                        $("#abc>h1").text(ts);
                        if (err != "no") {
                            $("#abc tbody").append("<tr><td>登录账号</td><td>联系电话</td><td>代表团名称</td><tr>")
                            $.each(err, function (k, v) {
                                $("#abc tbody").append("<tr><td>" + v.account + "</td><td>" + v.tel + "</td><td>" + v.allname + "</td></tr>")
                ...
                $("#uesrupfile").ajaxSubmit({
                    ...
                    success: function (json) {
                        $.get("/delegation/excel.jsp", {'n': json.url}, function (e) {
                            ...
                            state = "4";
                            if (d != 0) {
                                ts = "共" + c + "条数据,成功导入" + b + "条数据," + d + "条未成功导入,可能原因为默认登录账号(手机号码、代表团)已存在或导入格式不正确,未成功导入信息如下"
                            } else {
