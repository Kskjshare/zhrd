    ...
    <body>
        <form method="post" id="mainwrap">
            <div class="toolbar">
                ...
                <!--<div style="display: inline; position: relative;"><i onclick="reload()" class="iconfont iconshuaxin" style="cursor: pointer; font-size:18px; color:#56c21e; position: relative; top: 4px; right:2px;"></i></div>-->
                <!--<input onclick="reload()" type="button" value="刷新" style="cursor: pointer; border:none; background-color: #ffffff; color: #2d6c86;" />-->
                ...
                <table class="wp100 tc cellbor" id="section">
                    <thead>
                        <tr>
                            ...
                            <th class="realcompanyname no">重点批评</th>
                            <!--zyx增加重点批评列-->
                                <%
                                } else {
                                %>
                            <th class="realcompanyname no">主办单位</th>
                            <th class="importance">重点批评</th>
                                <%
                                    }
                                %>
                            ...
                            switch (powerid) {
                                case "5":    //代表//环节1                           
                                    keywhere = "realname like '%" + URLDecoder.decode(req.get("realname"), "utf-8") + "%' and title like '%" +
                                            ...
                                            URLDecoder.decode(req.get("meetingnum"), "utf-8") + "%' and lwstate=3 and draft=2 and myid=" + cookie.Get("myid");
                                    break;
...
//                                            "%' and lwid like '%" + URLDecoder.decode(req.get("lwid"), "utf-8") + 
                                    ...
                                    keywhere += 
                                            ...
                                            "%' and lwstate=3 and draft=2 and isysw=21";
//                                    System.out.println("1------------------------------------" + keywhere);
                                ...
                                case "22": //代表团(审查)
                                    ...
                                    keywhere = "realname like '%" + URLDecoder.decode(req.get("realname"), "utf-8") + //---------------
                                            ...
                                            "%' and year like '%" + URLDecoder.decode(req.get("year"), "utf-8") + 
                                            "%' and meetingnum like '%" + URLDecoder.decode(req.get("meetingnum"), "utf-8") + 
                                            "%' and lwstate=3 and draft=2 and ismeet=1 and mission=" + cookie.Get("myid");
                                     }else{
                                           keywhere = "realname like '%" + URLDecoder.decode(req.get("realname"), "utf-8") + //---------------
                                            "%' and title like '%" + URLDecoder.decode(req.get("title"), "utf-8") + //------------
                                            "%' and methoded like '%" + URLDecoder.decode(req.get("methoded"), "utf-8") + 
                                            ...
                                            "%' and year like '%" + URLDecoder.decode(req.get("year"), "utf-8") + 
                                            "%' and meetingnum like '%" + URLDecoder.decode(req.get("meetingnum"), "utf-8") + 
                                            "%' and lwstate=3 and draft=1 and ismeet=0 and mission=" + cookie.Get("myid");
                                     }
                                    //梁小竹注释
//                                    System.out.println("11111111111111111111"+keywhere);
                                ...
                                case "8": //选联委(审查)或人代委  ding:议审委
                                    ...
                                    keywhere = "realname like '%" + URLDecoder.decode(req.get("realname"), "utf-8") + "%' and title like '%" + 
                                            ...
                                            "%' and lwstate=3 and draft=2 and  isysw=0";
                                ...
                                case "25": //乡镇(审查)//环节3/在flowtype为1代表团初审时流程，乡镇层次支流程
                                    ...
                                    keywhere = "realname like '%" + URLDecoder.decode(req.get("realname"), "utf-8") + 
                                            ...
                                            "%' and year like '%" + URLDecoder.decode(req.get("year"), "utf-8") + 
                                            "%' and meetingnum like '%" + URLDecoder.decode(req.get("meetingnum"), "utf-8") + 
                                            "%' and lwstate=3 and draft=2 ";
                                     //"%' and lwstate=3 and draft=2 and (fsrID=" + cookie.Get("myid")+" or xzscID=" + cookie.Get("myid")+")";
...
//                                            "%' and lwid like '%" + URLDecoder.decode(req.get("lwid"), "utf-8") + 
                                   //added by ding 汝州项目督察局只能审批市级代表，所以加一个level条件查找记录  
                                    ...
                                    keywhere += 
                                            ...
                                            "%' and realid like '%" + URLDecoder.decode(req.get("realid"), "utf-8") + 
                                            "%' and meetingnum like '%" + URLDecoder.decode(req.get("meetingnum"), "utf-8") + 
                                            "%' and lwstate=3 and draft=2 and examination =2";
                                     if ( meetingTime.equals("1") ) {
...
//                                    }
                                    keywhere = "realname like '%" + URLDecoder.decode(req.get("realname"), "utf-8") + "%' and title like '%" + URLDecoder.decode(req.get("title"), "utf-8") + 
                                            ...
                                            + "%' and lwid like '%" + URLDecoder.decode(req.get("lwid"), "utf-8") + "%' and importance like '%" +
                                            URLDecoder.decode(req.get("importance"), "utf-8") + "%' and ProposedBill like '%" + URLDecoder.decode(req.get("ProposedBill"), "utf-8") + "%' and year like '%" 
                                            + URLDecoder.decode(req.get("year"), "utf-8") + "%' and realid like '%" + URLDecoder.decode(req.get("realid"), "utf-8") + "%' and meetingnum like '%" 
                                            + URLDecoder.decode(req.get("meetingnum"), "utf-8") + "%' and lwstate=3 and draft=2 and deal =1 and companyid=" + cookie.Get("myid");
                                    if ( meetingTime.equals("1") ) {
                                        keywhere += " and ismeet= 1";
                                    }
                                    else {
                                ...
                                default://commented by jackie
                                    keywhere = "realname like '%" + URLDecoder.decode(req.get("realname"), "utf-8") + "%' and title like '%" + URLDecoder.decode(req.get("title"), "utf-8") + "%' and methoded like '%" + URLDecoder.decode(req.get("methoded"), "utf-8") + "%' and realcompanyname like '%" + URLDecoder.decode(req.get("realcompanyname"), "utf-8") + "%' and lwid like '%" + URLDecoder.decode(req.get("lwid"), "utf-8") + "%' and importance like '%" + URLDecoder.decode(req.get("importance"), "utf-8") + "%' and allname like '%" + URLDecoder.decode(req.get("allname"), "utf-8") + "%' and sessionid like '%" + URLDecoder.decode(req.get("sessionid"), "utf-8") + "%' and telphone like '%" + URLDecoder.decode(req.get("telphone"), "utf-8") + "%' and realid like '%" + URLDecoder.decode(req.get("realid"), "utf-8") + "%' and ProposedBill like '%" + URLDecoder.decode(req.get("ProposedBill"), "utf-8") + "%' and year like '%" + URLDecoder.decode(req.get("year"), "utf-8") + "%' and meetingnum like '%" + URLDecoder.decode(req.get("meetingnum"), "utf-8") + "%' and lwstate=3 and draft=2";
                                    break;
                            }
                            ...
                            <!--<td class="handle no">-->
                                <% //examination:    审查状态 1:未审查,2:已审查,3:置回,5:待审查,6:乡镇已审查
                                    ...
                                    if (list.get("examination").equals("2")) {
                                        ...
                                        btnAuditShow = 0 ;
                                        if ( list.get("handlestate").equals("2") || list.get("workflow").equals("1") ){ //建议议案办理状态1:未确定,2待确定,3已确定,4申退中
        ...
        <script>
            ...
            $(function(){
                ﻿$(".bianji").click(function(){
                    let id = $(this).attr("id");
                popuplayer.display("/billlist/editupdate_1.jsp?id=" + id + "&TB_iframe=true", '编辑', {width: 832, height: 560});
                })
            });
             $(function(){
                ﻿$(".delete").click(function(){
                    ...
                    popuplayer.display("/listManager/delete3.jsp?id=" + id + "&TB_iframe=true&myid="+myid, '删除', {width: 360, height: 100});
                })
            });
            $(function(){
...
{
    popuplayer.display("/listManager/quicksearch.jsp", '快速查询', {width: 500, height: 100});
}
//zyx重点
                         $(".shi").click(function () {
                            let id = $(this).attr("id");
