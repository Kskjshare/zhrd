        ...
        <form method="post" class="popupwrap">
            <div>
                <table class="wp100 cellbor">
                    <tr>
                        <td colspan="4" class="institle dce">该题信息</td>
                    </tr>
                    <tr>
...
<!--                        <td colspan="3"><input type="text" maxlength="80" class="b95" name="title" value="<% entity.write("title"); %>" /></td>-->
        ...
        <script>
            //定义一个多选的选项数量变量
            var duoNum = 4;
            let getoptions = '<%=entity.get("options") %>';
            let num = getoptions.split(".-f/,").length;
             duoNum = num;
            $(function duo(){
             
             var optionHtml = " ";
             var question = <%=entity.get("questiontype")%>
             
            if(question==1){
                    $(".Duo").remove();
                    $(".ISTure").remove();

                    let getValue = '<%=entity.get("options") %>'   //input选项 赋值
                    let arr = getValue.split(".-f/,");
                    let arr2 = [];
                    arr.forEach(item=>{
                    ...
                    for (var i = 0; i < 4; i++) {
                        ...
                        optionHtml+= '</span><input type="text" maxlength="80" class="b95" name="d'+(i+1)+'"  value='+arr2[i]+'></td></tr>'
                    }

                    $('.cellbor').append(optionHtml);

                    let getValue1 = '<%=entity.get("answer") %>'  //-----设置radio选中状态
                    // getValue=getValue.charCodeAt()-65;
                    getValue1 = getValue1.substring(0,1).charCodeAt() - 65;
                    $("[value='" + getValue1 + "']").prop('checked', true);
                    ...
                    for (var i = 0; i < duoNum; i++) {
                        ...
                        optionHtml+='<input type="text" maxlength="80" class="b95" name="d'+(i+1)+'" value='+ (i < num ? arr2[i] : "") + ' ></td></tr>'
                    ...
                    let getValue1 = '<%=entity.get("answer") %>'  //---设置checbox选中状态
                    getValue1 = getValue1.split(".-f/,");
                    getValue1.forEach(item=>{
                        let checkDuo = item.substring(0,1).charCodeAt() - 65;
                        $("[value='" + checkDuo + "']").prop('checked', true);
                    })
                    ...
                    let getValue = '<%=entity.get("answer") %>'
                    $("[value='" + getValue + "']").prop('checked', true); //---设置radio选中状态
...
// -------------------------------------------------------点击切换---------------------------------------------------------------------------------
            ...
            var iteValue = $("[name='questiontype']").val();
                console.log(iteValue,"uuuuuuuuuuuuuuuuuuuuuuu")
                if(iteValue==1){
                    optionHtml="";
                         $(".duoyu2").remove();
                        $(".ISTure").remove();
