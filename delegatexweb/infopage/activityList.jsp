...
<html>
    ...
    <body>
        ...
        <script>
              RssApi.View.List("activities_list").keyvalue({ "pagesize": 100000}).condition({}).getJson(function (json) {  //这里是返回得json数据
console.log(json);
                 var str ="";
                  str ='<tr><td>序号</td><td>所属乡镇（街道）</td><td>站点名称</td><td>操作</td></tr>'
                  let rowspan = json.length ;
                  var opretion = 1;
                 json.forEach(( item , idex ) => {
                                        var opentime = item.opentime;

                    str += '<tr><td>'+ opretion+'</td><td>'+ item.realname +'</td> <td>'+ item.title +'</td>' + '<td><span  stationid="'+ item.stationid+'"><a href="/delegatexweb/infopage/delegationinfo.jsp?stationid='+item.stationid+'">点击进入</span></td></tr>';
//                    str += '<tr><td>'+opretion+'</td><td rowspan="'+rowspan+'">'+ item.allname+'</td><td>' + item.title+'</td>' + '<td>'+ opentime+'</td>  <td rowspan="'+rowspan+'"><span  stationid="'+ item.stationid+'"><a href="/delegatexweb/infopage/delegationinfo.jsp?stationid='+ item.stationid + '">点击进入</span></td></tr>';
                    opretion ++ 
                    $('#siteForm').html(str) 
