...
<html>
    <head>
        ...
        <style>
            ...
            #infotable>h1{background: #f7f7f7;line-height: 42px;font-size: 20px;text-align: center;font-weight: 400;margin: 0;}
            #infotable>table{width: 1018px;margin: 20px auto;font-size: 14px;border: #eee solid thin;border-width:1px 0px 0px 1px;}
    ...
    <body>
        ...
        <script>
//            RssApi.Table.List("contactstation_sub").keyvalue({ "pagesize": 100 ,"demonstration":3}).getJson(function (data) {
            RssApi.Table.List("contactstation_sub").condition({ "demonstration":4 }).keyvalue({ "pagesize": 100 }).getJson(function (data) {
//            RssApi.View.List("contactstation_sub").keyvalue({ "demonstration": 3}).condition({stationid:item.id}).getJson(function (data) { 
            var str ="";
            //str ='<tr><td>序号</td><td>所属乡镇（街道）</td><td>站点名称</td><td>站长</td><td>站长电话</td><td>联络员</td><td>联络员电话</td></tr>'
            str ='<tr ><td>序号</td><td>站点名称</td><td>站长</td><td>站长电话</td><td>联络员</td><td>联络员电话</td><td>操作</td></tr>'
            opretion = 1;
                    if(data.length){
...
//                                     str += '<tr><td>'+opretion+'</td><td rowspan="'+rowspan+'">'+ele.name+'</td><td>'+ele.title+'</td><td>'+ele.master+'</td> <td>'+ele.mastertelephone+'</td> <td>'+ele.linkman+'</td>  <td>'+ele.linkmantelephone+'</td> '+'  <td rowspan="'+rowspan+'"><span  stationid="'+ele.stationid+'"><a href="/delegatexweb/infopage/delegationinfo.jsp?stationid='+ele.stationid+'">点击进入</span></td></tr>';
//                                }else{
//
//                                    str += '<tr><td>'+opretion+'</td><td>'+ele.title+'</td>  <td>'+ele.master+'</td> <td>'+ele.mastertelephone+'</td> <td>'+ele.linkman+'</td>  <td>'+ele.linkmantelephone+'</td>' + '</tr>'
//                                }
                                
//                                    str += '<tr><td>'+opretion+'</td><td>'+ele.title+'</td>  <td>'+ele.master+'</td> <td>'+ele.mastertelephone+'</td> <td>'+ele.linkman+'</td>  <td>'+ele.linkmantelephone+'</td>' + '</tr>'
//                                     str += '<tr><td>'+opretion+'</td><td "'+'">'+ele.title+'</td>'+'<td>'+ele.master+'</td> <td>'+ele.mastertelephone+'</td> <td>'+ele.linkman+'</td>  <td>'+ele.linkmantelephone+'</td> '+'  <td "'+'"><span  "'+'"><a href="/delegatexweb/infopage/delegationinfo.jsp?stationid='+ele.stationid+'">点击进入</span></td></tr>';
                                     str += '<tr><td>'+opretion+'</td><td "'+'">'+ele.title+'</td>'+'<td>'+ele.master+'</td> <td>'+ele.mastertelephone+'</td> <td>'+ele.linkman+'</td>  <td>'+ele.linkmantelephone+'</td> '+'  <td "'+'"><span  "'+'"><a href="/delegatexweb2/contactstation.htm?myid='+ele.substationid+'">点击进入</span></td></tr>';
