...
<%
    ...
    if (!entity.get("action").isEmpty()) {
        entity.remove("scyj", "realid", "perm", "type1", "type2", "type3", "type4", "blfs", "secound", "contacts", "usercomp", "scr","num");
        entity.remove("id");
        ...
        <form method="post" class="popupwrap">           
            <div>
                ...
                <table class="wp100 cellbor">
                    ...
                    <tr>
                        <td class="w150 dce">建议标题<em class="red">*</em></td>
                    ...
                    <tr>
                        ...
                        <td><b class="selectuser">添加附议人</b></td>
                    </tr>
                    ...
                    <tr><td>附件</td><td colspan="4" class="wj">
<!--                            <div><span id="fjfile<%=num%>" rid="<%=num%>"><% out.print(arry[0].isEmpty() ? "点击选择文件" : arry[0]); %></span><input type="hidden" id="enclosure1" value="<% out.print(arry[0]); %>" ></div>-->
...
//                alert($('[name=draft]').val());
            ...
            //上传附件
            var res =parseInt($("input[name='num']").val());
            $("span[rid]").click(function () {
//                var rid = $(this).attr("rid")
                $("#file" + res).click();
            })
            
            $("#fileeform1>input").change(function () {
                alert(res);
                if (res == "1") {
                    res++;
                    $(".fm").append("<form id='fileeform" + res + "' class='fileeform' enctype='multipart/form-data' method='post'><input type='file' id='file" + res + "' ridform='" + res + "'  name='file" + res + "' multiple tabindex='"+res+"'></form>");
                    res=1;
                }else{
                    $(".fm").append("<form id='fileeform" + res + "' class='fileeform' enctype='multipart/form-data' method='post'><input type='file' id='file" + res + "' ridform='" + res + "'  name='file" + res + "' multiple tabindex='"+res+"'></form>");
                }
                var tt = $(".wj");
                tt.append("<div><span id='fjfile" + res + "'><% out.print(arry[0].isEmpty() ? "点击选择文件" : arry[0]); %></span><input type='hidden' id='enclosure" + res + "' value='<% out.print(arry[0]); %>' ></div>");
...
//                var aa = parseInt($("input[name='num']").val());
//                $("input[name='num']").val(aa + 1);
//                var z = $("input[name='num']").val();
//                res = z;
//                $("span[rid]").attr("rid", res);
                res++;
                $("span[rid]").attr("rid",res);
                return false;
            })
//            $("#fileeform2>input").change(function () {
//                alert(res);
//                if (res == "1") {
//                    res++;
//                    $(".fm").append("<form id='fileeform" + res + "' class='fileeform' enctype='multipart/form-data' method='post'><input type='file' id='file" + res + "' ridform='" + res + "'  name='file" + res + "' multiple tabindex='"+res+"'></form>");
...
//            })
            ...
            $('.selectuser').click(function () {
                ...
                RssWin.onwinreceivemsg = function (dict) {
                    ...
                    $.each(dict, function (k, v) {
                        if ($("em[myid='" + v.myid + "']").length == "0") {
                            t.append("<li myid='" + v.myid + "'><em  style='display: inline;width: 18.2%;height: 100%;border: 1px #dce6f5 dashed;float: left;border-top: 0px;border-right: 0px;border-left: 0px;'>" + v.realname + "</em><em style='inline block;width: 32%;height: 100%;border: 1px #dce6f5 dashed;float: left;border-top: 0px;border-right: 0px;'>&nbsp;" + v.code + "</em><em style='display: inline;width: 33%;height: 100%;border: 1px #dce6f5 dashed;float: left;border-top: 0px;border-right: 0px;'>&nbsp;" + v.homeaddress + "</em><em style='display: inline;width: 12%;height: 100%;border: 1px #dce6f5 dashed;float: left;border-top: 0px;border-right: 0px;'>&nbsp;" + v.telphone + "</em><em class='red'>删除</em></li>")
