...
<html>
    ...
    <body>
        ...
        <ul id="right">
            <li id="top">
                <h1>统计设置</h1>
<!--                <label><input type="radio" name="state" memutype="1">宗教统计</label>
                ...
                <label><input type="radio" name="state" memutype="7">民族统计</label>-->
...
<script>
    ...
    $("[name='state']").click(function () {
        ...
        new Ajax("delegatestatistics").keyvalue({"state": memutype}).getJson(function (json) {
            ...
            switch (memutype) {
                case "1":
                    bingtitle = {text: '宗教统计', x: 'center'}
                    var table = "<thead><tr><th class='w70'>序号</th><th >宗教</th><th>人数</th></tr></thead><tdody>";
                    var num = 1;
                    var counters = 0 ;
                    var i = 0 ;
                    ...
                    for ( i = 0 ; i < 6 ;i ++ ){
                       if ( i == 0 ) {
                        ...
                        bingoption.push({value: counters, name: dictdata["religion"][i]})
                        tiaooption.push(dictdata["religion"][i]);
                        tiaodate.push( counters );
                       
                        table += "<tr><td class='num'>" + num + "</td><td>" + dictdata["religion"][i] + "</td><td>" + counters + "</td>"
                        num++;
                    }
...
//                    })
                    ...
                    break;
                case "2":
                    bingtitle = {text: '性别统计', x: 'center'}
                    var table = "<thead><tr><th>序号</th><th >性别</th><th >人数</th></tr></thead><tdody>";
                    var num = 1;
                    $.each(json, function (k, v) {                      
                        if ( v.sex == null ) {                          
                        }
                    ...
                    break;
                case "3":
                    bingtitle = {text: '学历统计', x: 'center'}
                    var table = "<thead><tr><th>序号</th><th>学历</th><th >人数</th></tr></thead><tdody>";
                    var num = 1;
                    $.each(json, function (k, v) {
                        if ( v.eduid == null ) {                          
                        }
                    ...
                    break;
                case "4":
                    bingtitle = {text: '学位统计', x: 'center'}
                    var table = "<thead><tr><th>序号</th><th >学位</th><th >人数</th></tr></thead><tdody>";
                    var num = 1;
                    $.each(json, function (k, v) {
                        if (  v.degree == null ) {                          
                        }
                    ...
                    break;
                case "5":
                    bingtitle = {text: '党派统计', x: 'center'}
                    var table = "<thead><tr><th>序号</th><th >党派</th><th >人数</th></tr></thead><tdody>";
                    var num = 1;
                    $.each(json, function (k, v) {
                        if (  v.clan == null ) {                          
                        }
                ...
                case "6":
                    ...
                    break;
                case "7":
                    bingtitle = {text: '民族统计', x: 'center'}
                    var table = "<thead><tr><th>序号</th><th >民族</th><th >人数</th></tr></thead><tdody>";
                    var num = 1;
                    var cnt = 1 ;
                    $.each(json, function (k, v) {
                        ...
                        if ( cnt == 1 ) {
                            bingoption.push({value: 98, name: dictdata["nationid"][cnt]})
                            tiaooption.push( dictdata["nationid"][cnt] );
                            tiaodate.push( 98 );
                        }else {
                            bingoption.push({value: v.num, name: dictdata["nationid"][cnt]})
                            tiaooption.push( dictdata["nationid"][cnt] );
                            tiaodate.push(v.num);
                        }
                        table += "<tr><td>" + num + "</td><td>" + dictdata["nationid"][cnt] + "</td><td>" + v.num + "</td>"
                        num++;
                        cnt ++ ;
                       
                    })
                    
                    for ( var i = cnt  ; i < 58 ; i ++ ) {
                        //bingoption.push({value: 1, name: dictdata["nationid"][i]})
                        tiaooption.push( dictdata["nationid"][cnt] );
                        tiaodate.push( 0 );
                        table += "<tr><td>" + num + "</td><td>" + dictdata["nationid"][i] + "</td><td>" + 0 + "</td>"
                        num++;
                        cnt ++ ; 
                    }
