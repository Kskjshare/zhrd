                ...
                <table class="wp100 tc cellbor" id="section">
                    ...
                    <tbody>
                        <%
                            ...
                            sql = "delegationname like '%" + URLDecoder.decode(list.get("delegationname"), "utf-8") + "%' and telphone like '%" + URLDecoder.decode(list.get("telphone"), "utf-8") + "%' and realname like '%" + URLDecoder.decode(list.get("realname"), "utf-8") + "%' and sex like '%" + URLDecoder.decode(list.get("sex"), "utf-8") + "%' and nationid like '%" + URLDecoder.decode(list.get("nationid"), "utf-8") + "%' and birthday like '%" + URLDecoder.decode(list.get("birthday"), "utf-8") + "%' and sessionlist like '%" + URLDecoder.decode(list.get("sessionid"), "utf-8") + "%' and isdelegate=1";
                            sql = "isdelegate=1 and mission=" + UserList.MyID(request);

                            list.select().where(sql).get_page_desc("myid");
                            
                            boolean istowndelegate = false ;
                            while (list.for_in_rows()) {
                            istowndelegate = false ;
        
                            istowndelegate = list.get("circleslist").contains("4");    
                        ...
                        <tr ondblclick="ck_ondblclick();" idd="<% out.print(list.get("myid")); %>" style="cursor:pointer;">
                            ...
                            <td><%
                                ...
                                String[] circlesLevel = {"全国人大代表", "省人大代表", "平顶山市人大代表", "市人大代表", "乡镇人大代表"};//所属层次
                            ...
                            <!--<td sessionclassify="<% list.write("sessionid"); %>">-->
<!--                            <td>
                                <%
                                    String[] arry1 = list.get("sessionlist").split(",");
