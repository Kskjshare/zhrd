                ...
                <table class="wp100 tc cellbor" id="section">
                    ...
                    <tbody>
                        <%
                            ...
                            //list.select().where("title like '%" + URLDecoder.decode(list.get("title"), "utf-8") + "%'and state like '%" + URLDecoder.decode(list.get("state"), "utf-8") + "%'" + "and taskDone=1 and typeid=5 and myid="+UserList.MyID(request)).get_page_desc("id");
//                            list.select().where("title like '%" + URLDecoder.decode(list.get("title"), "utf-8") + "%'and state like '%" + URLDecoder.decode(list.get("state"), "utf-8") + "%'" + "and taskDone=1 and typeid=5").get_page_desc("id");
                            list.select().where("title like '%" + URLDecoder.decode(list.get("title"), "utf-8") + "%'and state like '%" + URLDecoder.decode(list.get("state"), "utf-8") + "%'" + "and taskDone=1 and typeid=5").get_page_desc("id");
