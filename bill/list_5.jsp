...
if (meetingTime.equals("0")){
            list.select().where("lwstate=5 and draft=2 and ismeet=0 and mission=" + cookie.Get("myid")).get_page_desc("id");
    }
    else {
             list.select().where("lwstate=5 and draft=2 and ismeet=1 and mission=" + cookie.Get("myid")).get_page_desc("id");
    }
//                            
//                            if (meetingTime.equals("0")) {
//                                if ( list.get("level").equals("0") )
//                                list.select().where("draft=2 and lwstate="+ list.get("lwstate")  + " and level=0" +" and ismeet= 0").get_page_desc("realid");
//                                else
//                                   list.select().where("draft=2 and lwstate="+ list.get("lwstate")  + " and level=1"+" and ismeet= 0").get_page_desc("realid"); 
//                            }else{
//                                //list.select().where("draft=2 and lwstate="+ list.get("lwstate")  + " and level="+list.get("level") +" and ismeet= 1").get_page_desc("realid");
//                                if ( list.get("level").equals("0") )
//                                list.select().where("draft=2 and lwstate="+ list.get("lwstate")  + " and level=0" +" and ismeet= 1").get_page_desc("realid");
//                                else
//                                   list.select().where("draft=2 and lwstate="+ list.get("lwstate")  + " and level=1"+" and ismeet= 1").get_page_desc("realid"); 
////                            }
                             ...
                             <!--<td class="handle no">-->
                                <%
                                    ...
                                    if (list.get("examination").equals("1")) {
                                       // handle = "未审查";
                                        handle = "<td class='handle no' style='color:red'></td>";
                                    }
        ...
        <script>
             ...
             $(function(){
                ﻿$(".edit").click(function(){
                    let id = $(this).attr("id");
                popuplayer.display("/billlist/editupdate_3.jsp?id=" + id , '编辑', {width: 830, height: 560});
                })
            });
             $(function(){
