...
//    }
    ...
    //rank_sort_list.select().where("myid=" +  cookie.Get("myid") ).get_page_desc("id"); 
    rank_sort_list.select().where("myid=" + cookie.Get("myid") ).get_first_rows();
...
//        int a = sort.totalrows + 1;
        ...
        notify_messages.keyvalue("lwstate", 1 );
       ...
       //// notify_messages.append().submit(); ///////////////////////////////20220520 删除
        ...
        entity3.keyvalue("relationid", entity.autoid ); //为了处理删除记录同时删除newinformation_list的数据，20220512不知道会不会有问题
...
//        rank_sort_list.remove("scyj", "realid", "perm", "type1", "type2", "type3", "type4", "blfs", "secound", "contacts", "usercomp", "scr");
//        rank_sort_list.remove("id");
//        rank_sort_list.remove("secound");
//        rank_sort_list.remove("classifyid", "title", "keywords", "summary", "matter", "shijian", "ico", "blfs", "url", "audio", "audioduration", "video"
//                              , "auditremark", "remarks", " picture", "price", "coorganiserid", "integral", "onlineamount", "viewamount", "commentamount", "replymyid", 
//                              "infotype", "realname", "matter", "title", "notice", "repertory", "year", "permission", "buyamount", "crowd",
//                              "auditremark", "remarks", "praiseamount", "iscomment", "meetingnum", "repertory", "year", "permission", "buyamount", "crowd",
//                              "discussinspecttime", "gradeavg", "opinioned", "organizer", "seconded", "methoded", "reviewclass", "draft", "lwstate", "handle",
//                              "registertype", "importance", "excellent", "leader", "examination", "realcompanyid", "undertake", "sessionid", "discussenclosurename", "discussenclosure"
//                              );
//
//          rank_sort_list.remove("sessionid", "lwstate", "delegationname", "type1", "type2", "type3", "type4", "matter", "type5", "contacts", "usercomp", "scr",
//                  "usercomp", "numpeople", "ProposedBill", "draft", "year", "type4", "scr", "type5", "contacts", "usercomp", "scr",
//                  "enclosure", "enclosurename", "reviewclass", "perm", "permission", "sugformation", "sugyears", "written", "seconded", "secound", "contacts");
//                                   
//        
        
        
        if ( rank_sort_list.pagerows > 0 ) { //用户存在
            int suggestcnt =  0 ;
            int num = 0 ;
            int totalScore = 0 ;
            

            suggestcnt = Integer.parseInt ( rank_sort_list.get("suggest") );
            num = Integer.parseInt ( rank_sort_list.get("num") );
            totalScore = Integer.parseInt ( rank_sort_list.get("totalScore") );
            if ( suggestcnt >= 1 ) {
                totalScore ++ ;
            }
            else {
                totalScore += suggestBaseScore ;
            }
            suggestcnt ++ ;
            num ++ ;
            rank_sort_list.keyvalue("num", num ); //总数
            rank_sort_list.keyvalue("suggest", suggestcnt ); //总数
            rank_sort_list.keyvalue("totalScore", totalScore ); 


            //rank_sort_list.update().where("myid=" + req.get("myid") ).submit();
//            rank_sort_list.update().where("myid=" + UserList.MyID(request) ).submit();
//            second.delete().where("suggestid=" + entity.get("id")).submit();
        }
        else {
//            rank_sort_list.keymyid( UserList.MyID(request) );
//            rank_sort_list.keyvalue("totalScore", suggestBaseScore +"" );
//            rank_sort_list.keyvalue("num", 1  +"" ); //总数
//            rank_sort_list.keyvalue("suggest", 1 +"" );
//            rank_sort_list.append().submit();
        }
...
-->
                ...
                <h1 style="font-size:22px;font-family: 微软雅黑">人民代表大会代表建议</h1>
                <table class="wp100 cellbor">
                    ...
                    <tr>
                        ...
                        <!--  
                      <td colspan="2" style="font-size:16px"><select style="font-size:16px;font-family: 微软雅黑" class="w260" name="sessionid" dict-select="sessionclassify" def="<% entity.write("sessionid"); %>"></select></td>
                        -->                        
                      ...
                      <td colspan="2" style="font-size:16px">
                          <select style="font-size:16px;font-family: 微软雅黑" class="w99" name="sessionid" dict-select="sessionclassify" def="<% entity.write("sessionid"); %>"></select></td>
...
<!--                            <select style="font-size:16px;font-family: 微软雅黑" type="text" maxlength="80" class="w260" name="level" dict-select="circles" def="<% entity.write("level"); %>" >
                        ...
                        <td colspan="2" id="parentname" style="font-size:16px"><input style="font-size:16px;font-family: 微软雅黑" class="yincang" type="text" value="<% user.write("delegationname"); %>" ></td>
                    </tr>
                    <tr>
...
<!--                            <select type="text" maxlength="80" class="w260" name="reviewclass" dict-select="companytypeeclassify" def="<% entity.write("reviewclass"); %>" ></select>-->
                    ...
                    <tr  class="fsry">
                        ...
                        <td  style="font-size:16px;font-family: 微软雅黑">审查单位</td>
