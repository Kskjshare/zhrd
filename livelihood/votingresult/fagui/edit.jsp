        ...
        switch (entity.get("action")) {
            case "append":
                ...
                while (senduser.for_in_rows()) {
                    ...
                    jpushClient.sendPush(PushPayload.newBuilder().setPlatform(Platform.android()).setAudience(Audience.all()).setNotification(Notification.android("您有一条新的法规文案！","法规文案",map)).setOptions(Options.newBuilder().setApnsProduction(false).setSendno(ServiceHelper.generateSendno()).setTimeToLive(86400).build()).setMessage(Message.content("您收到一条法规文案")).build());
                    }
                }
                break;
            case "update":
