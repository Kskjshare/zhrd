...
public class MeetingServiceImpl implements MeetingService {
	...
	@Resource
	MeetMainDao meetMainDao;
	@Resource
	MeetYichenMainDao yichenMainDao;
	@Resource
	MeetYichenFileDao yichenFileDao;
	@Resource
	MeetRichenMainDao richenMainDao;
	@Resource
	MeetRichenYitiDao richenYitiDao;
	@Resource
	MeetFileMainDao meetFileMainDao;
	@Resource
	MeetGroupMainDao meetGroupMainDao;
	@Resource
	MeetProcessDao meetProcessDao;
	@Resource
	MeetProcessLogDao meetProcessLogDao;
	@Resource
	MeetFileScopeDao filescopeDao;
	@Resource
	DictDataService dictDataService;
	@Resource
	MeetingProcPubService meetPubService;
	@Resource
	RemindService remindService;
	@Resource
	MeetPubFileDao pubMeetFileDao;
	@Resource
	MeetPubGroupDao meetPubGroupDao;
	@Resource
	MeetingViewService meetingViewService;
	@Resource
	MeetRichenFileDao meetRichenFileDao;
	@Resource
	MeetFileUserDao meetFileUserDao;
	@Resource
	SysScopeUserDao sysScopeUserDao;
	...
	public String deleteBashYitiAndRichen(String ids, String filetypes) {
		...
		if (ids != null && ids.trim().length() != 0) {
			...
			if (arrayids.length != arrayfiletypes.length) {
				System.err
						.println("delete bash yiti and richen,length is not equals.");
				return "lengtherror";
			...
			for (int i = 0; i < arrayids.length; i++) {
				if (arrayfiletypes[i] != null) {
					if (arrayfiletypes[i].trim().equals("0")
							|| arrayfiletypes[i].trim().equals("4")) {
						yichenMainDao.deleteByPrimaryKey(Integer
								.parseInt(arrayids[i]));
						yichenFileDao.deleteYichFileByYichenid(Integer
								.parseInt(arrayids[i]));
					}

					if (arrayfiletypes[i].trim().equals("5")
							|| arrayfiletypes[i].trim().equals("6")) {
						richenMainDao.deleteByPrimaryKey(Integer
								.parseInt(arrayids[i]));
						richenYitiDao.deleteYitiByRichenId(Integer
								.parseInt(arrayids[i]));
					}

					if (arrayfiletypes[i].trim().equals("7")) {
						richenYitiDao.deleteByPrimaryKey(Integer
								.parseInt(arrayids[i]));
					}
				}
	...
	public void AutoGeneMeetYichen(String filename, int meetid) {
		...
		List<Map<String, Object>> list = WordToYichenUtil
		...
		for (int i = 0; i < list.size(); i++) {
			...
			MeetYichenMainEntity temp = yichenMainDao
					.selectYichenByTitleAndMeetid(map);
	...
	public void delMeetYichen(int yichenid) {
		// 如果是已发布状态，则更新状态
		MeetYichenMainEntity yichen = yichenMainDao
				.selectByPrimaryKey(yichenid);
		meetPubService.chgMeetProcState("yichen", yichen.getMeetid());
	...
	public void AutoGeneMeetRichen(String filename, int meetid) {
		...
		List<RichenConvertObject> list = WordToRichenUtil
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (dateName != null && dateName.trim().length() != 0) {
				...
				tempRichen = richenMainDao
						.selectParentRichenByNameAndMeetid(map);
				if (tempRichen != null) {
	...
	public void deleteRichen(int richenid) {
		...
		// 如果是已发布状态，则更新状态
		MeetRichenMainEntity richen = richenMainDao
				.selectByPrimaryKey(richenid);
		meetPubService.chgMeetProcState("richen", richen.getMeetid());
	...
	public void addRichenYiti(MeetRichenYitiEntity richyiti) {
		...
		MeetRichenMainEntity entity = richenMainDao.selectByPrimaryKey(richyiti
				.getRichenid());
	...
	public void deleteRichenYiti(int yitiid) {
		...
		// 更新流程状态
		MeetRichenMainEntity entity = richenMainDao
				.selectMeetIdByRichenYiyiid(yitiid);
		meetPubService.chgMeetProcState("richen", entity.getMeetid());
	...
	public MeetFileMainEntity addMeetFile(MeetFileMainEntity meetfile) {
		...
		int t = this.getMeetFileMaxSort(meetfile.getMeetid(),
				meetfile.getFileown());
		t = t + 5;
	...
	public MeetFileMainEntity addMeetFileHzzrhy(MeetFileMainEntity meetfile) {
		...
		if (filescope != null && filescope.trim().length() != 0) {
			...
			for (int i = 0; i < scopes.length; i++) {
				List<SysScopeUserEntity> selectByScopeId = sysScopeUserDao
						.selectByScopeId(Integer.valueOf(scopes[i]));
		...
		if (fileids != null && fileids.trim().length() != 0) {
			...
			for (int i = 0; i < arraystr.length; i++) {
				if (arraystr[i] != null && arraystr[i].trim().length() != 0) {
					...
					MeetFileMainEntity entity = meetFileMainDao
	...
	public Map<String, Object> autoRelaMeetFileAndYichen(int meetid) {
		...
		List<MeetYichenMainEntity> listyichen = yichenMainDao
				.selectAllByMeetid(meetid);
		...
		map.put("meetid", meetid);
		map.put("fileown", "1");
		List<MeetFileMainEntity> listfile = meetFileMainDao
				.selectAllFileByMeetidAndFileOwn(map);
		filetotal = listfile.size();

		List<MeetRichenYitiEntity> listyiti = richenYitiDao
				.selectYitiByMeetid(meetid);
		...
		for (int i = 0; i < listyichen.size(); i++) {
				...
				if (!"3".equals(file.getFiletype())) {
					...
					for (int k = 0; k < keywords.size(); k++) {
						if (fileTitle.indexOf(keywords.get(k)) >= 0) {
							...
							if (b
									&& sucesslist.contains(file.getFileid()
											.toString().trim()) == false) {
								bindsucess++;
								sucesslist.add(file.getFileid().toString()
										.trim());
								b = false;
							}

							temp = yichenFileDao
									.selectByYichenFileEntity(record);
							if (temp != null) {
							...
							if (log.isInfoEnabled()) {
								log.info("bind yichen and file,file is:"
										+ file.getFilename());
								log.info("bind yichen and file,yichen is:"
										+ yichen.getTitle());
							}
							break;
						}
			...
			System.out.println("bind yiti and yichen ,yiti list is:"
					+ listyiti.size());
			for (int t = 0; t < listyiti.size(); t++) {
				...
				for (int k1 = 0; k1 < keywords.size(); k1++) {
					...
					if (yitiname.indexOf(keywords.get(k1)) >= 0) {
						System.out
								.println("------bindyichenid.indexOf(yichen.getYichenid())--bindyichenid--"
										+ bindyichenid);
						System.out
								.println("------bindyichenid.indexOf(yichen.getYichenid())-yichen.getYichenid()-"
										+ yichen.getYichenid());
						System.out
								.println("------bindyichenid.indexOf(yichen.getYichenid())-yichen.getYichenid()-"
										+ yichen.getYichenid());

						if (bindyichenid != null
								&& bindyichenid.trim().length() != 0) {
							System.out
									.println("------bindyichenid.indexOf(yichen.getYichenid())-isindexof-"
											+ bindyichenid.indexOf(yichen
													.getYichenid().toString()));

							if (bindyichenid.indexOf(yichen.getYichenid()
									.toString()) >= 0) {
								break;
							} else {
	...
	public int upddateMeetGrp(MeetGroupMainEntity grp) {
		...
		if (grpno != null && grpno.trim().length() != 0) {
			...
			MeetGroupMainEntity group = meetGroupMainDao
					.selectGroupByMeetidAndGroupno(map);
			if (group == null) {
	 ...
	 * @return
	 */
	private MeetProcessEntity getCurMeetProcessByState(int meetid,
			String submodulecode, String curState) {
		...
		processEntity = meetProcessDao
				.selectCurProcessByMeetidAndSubmodulecode(map);
		...
		System.out.println("get current processEntity commint is:"
				+ processEntity.getComment());

		if (listprocess != null && listprocess.size() > 0) {
			processEntity.setCurstate(listprocess.get(0).getCurstate());
			String stateName = dictDataService.getNameByTypeAndcode(
					"procstate", processEntity.getCurstate());
			...
			for (int i = 0; i < listprocess.size(); i++) {
				...
				actionName = dictDataService.getNameByTypeAndcode("procaction",
						listprocess.get(i).getAction());
	...
	/**
	 * 提交当前流程状态----submodule yichen richen file huizhong bimu fenzu
	 */
	@Override
	public boolean submitMeetPub(int meetid, String submodule, String noTX) {
		...
		try {
			if (submodule != null && submodule.trim().equals("yichen")) {
				...
				if (!(noTX != null && noTX.trim().equals("noTX"))) {
					remindService
							.MeetFileRemindHandler("insert", fileentity, 0);
				}
	...
	public boolean updateSort(int id, String filetype, int sort) {
		...
		try {
			// 议程
			if (filetype != null
					&& (filetype.trim().equals("0") || filetype.trim().equals(
							"4"))) {
				map.put("yichenid", id);
				map.put("sort", sort);
				...
				// 日程
					...
					&& (filetype.trim().equals("5") || filetype.trim().equals(
							"6"))) {
				map.put("richenid", id);
				map.put("sort", sort);
				...
				// 文件管理
					...
					&& (filetype.trim().equals("1")
							|| filetype.trim().equals("2")
							|| filetype.trim().equals("3")
							|| filetype.trim().equals("8")
							|| filetype.trim().equals("9") || filetype.trim()
							.equals("10"))) {
				map.put("fileid", id);
				map.put("sort", sort);
	...
	public int selectFileCountByMeetidAndFileOwn(Map<String, Object> map) {
		int count = 0;
		try {
			count = meetFileMainDao
					.selectFileCountByMeetidAndFileOwnAndUserid(map);
		} catch (Exception e) {
			e.printStackTrace();
			count = 0;
		}
