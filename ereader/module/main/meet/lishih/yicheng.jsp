...
<body>
	<div class="cwh_head">议程</div>
	<!--此处内容更替-->
	<div class="tongzhi">
		<div class="tongzhi_main">
			<div style="width:98%;margin:0px auto;height:550px;overflow-y:auto;">
 			<table cellpadding="0" cellspacing="0" border="0" width="100%" style="table-layout:fixed;">
				<s:iterator value="yichenglist" var="fir">
					<s:if test="#fir.pid==0">
					<div style="display:none" id='<s:property value="#fir.id" />'></div>
						<tr name="firsttr" class="td_historyfirst" >
							<td width="96%" style="word-break:keep-all; white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"><span title="<s:property value="#fir.name" />"><s:property value="#fir.name" /></span></td>
							<td width="4%">
						...
						<s:iterator value="yichenglist"  var="sec">
							<s:if test="#sec.pid==#fir.id">
								<tr name="<s:property value="#sec.pid" />" style="display: none;" class="td_historysecond">
							       <td width="96%" style="word-break:keep-all; white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"><s:if test='#sec.filepath!=null'><a href="<%=basePath %>upload/<s:property value="#sec.filepath"/>" style="cursor: hand;" target="_blank" title="<s:property value="#sec.name" />"><s:property value="#sec.name" /></a></s:if>
							       	<s:if test='#sec.filepath==null'><span title="<s:property value="#sec.name" />"><s:property value="#sec.name" /></span></s:if>
							       </td>
								   <td width="4%" style="position: relative;">
									...
									<s:iterator value="yichenglist"  var="thir">
										<s:if test="#thir.pid==#sec.id">
											<tr name="<s:property value="#thir.pid" />" style="display: none;" class="td_historythird">
										       <td width="96%" style="word-break:keep-all; white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"><s:if test='#thir.filepath!=null'><a href="<%=basePath %>upload/<s:property value="#thir.filepath"/>" style="cursor: hand;" target="_blank" title="<s:property value="#thir.name" />"><s:property value="#thir.name" /></a></s:if>
											   ...
											   <td width="4%" style="position: relative;">
												...
												<s:iterator value="yichenglist"  var="four">
