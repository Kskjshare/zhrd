				...
				view.add(nodes).droppable({
					...
					onDrop:function(e,source){
						...
						if (tr.length){
							...
							dRow = getRow(tr);
							console.info("目标位置的fileType："+dRow.filetype);
							console.info("拖动的条目的fileType："+sRow.filetype);
							console.info(sTarget);
							console.info("dTarget"+dTarget);
							if(sRow.filetype-dRow.filetype>1)  
		                        {  
		                            tr.removeClass('treegrid-row-append treegrid-row-top treegrid-row-bottom');  
		                            alert("无法将文件拖动至此。");
		                            return;  
						...
						function insert(){
							var data = $(sTarget).treegrid('pop', sRow[opts.idField]);
							if (point == 'append'&&sRow.filetype>dRow.filetype){
								if (dRow){									
										$(dTarget).treegrid('append', {
											parent: dRow[opts.idField],
											data: [data]
