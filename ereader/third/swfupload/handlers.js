...
function fileQueued(file) {
	try {
		var scopeids=document.getElementById("selVal").value;
		//alert("scopeids:"+scopeids);
		if(scopeids==""||scopeids==null||scopeids=='undefined'){
			alert("请选择推送范围");
			isscope=false;
			return ;
		}else{
		...
		var progress = new FileProgress(file, this.customSettings.progressTarget); //file是选择的文件，progressTarget是显示区域div的id
...
function fileDialogComplete(numFilesSelected, numFilesQueued) {
	try {
		...
		if(isscope){
			...
			var scopeids=document.getElementById("selVal").value;
			//alert("fmtype:"+document.getElementById("fmtype").value+","+document.getElementById("fileown").value+","+scopeids);
			var postobj = { "meetid":document.getElementById("meetid").value,
					"fileown":document.getElementById("fileown").value,
					"fmtype":document.getElementById("fmtype").value,
					"scopeids":scopeids,"filetypeSel":document.getElementById("filetypeSel").value,};
...
function uploadSuccessXuhao(file, serverData) {
	try {
		var progress = new FileProgress(file, this.customSettings.progressTarget);//这怎么一直返回一。。。。纠结  action的setId设为动态值
		//progress.setComplete();
		progress.setStatus("上传完成");
		//alert(serverData);
		var jsonObj = JSON.parse(serverData);
	//	document.getElementById("reid").value=jsonObj.ID;
		...
		if(_filePath==undefined){
			...
			progress.setDownDeleteLink(_filePath,saveData,jsonObj.ID);
			progress.setFileType(jsonObj.ID,saveData);
			//progress.setCloseWin();
			//传值给页面
...
function uploadSuccess(file, serverData) {
	...
	try {
		var progress = new FileProgress(file, this.customSettings.progressTarget);//这怎么一直返回一。。。。纠结
		//progress.setComplete();
		progress.setStatus("上传完成");
	//	alert(serverData);
		var jsonObj = JSON.parse(serverData);
		//alert(jsonObj.filename);
