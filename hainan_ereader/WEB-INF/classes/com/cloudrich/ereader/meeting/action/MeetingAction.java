		...
		try{
			...
			if(file!=null){
				if(file.exists()){
					...
					groupMain.setFileguid(Md5GuidUtil.getMd5Guid(new File(upFile)));
				}
			}
...
//		return getRichenPage(type);
	...
	public void hzbenchAddByFile(){
		...
		try{
			...
			obj.setFileguid(Md5GuidUtil.getMd5Guid(new File(upFile)));
	...
	public void meetingfileadd(){
		try{
			try {
				...
				obj.setMeetid(Integer.parseInt(mid));
				obj.setFilescopes(scopeids);
				obj.setFileguid(Md5GuidUtil.getMd5Guid(new File(saveUploadFile(file, fileFileName,false))));
...
//					System.out.println("attachment=="+attachment.toString());
	...
	public void submitMeetProcess(){
		JSONObject json=new JSONObject();
		try{
			boolean b=meetingService.submitMeetPub(meetid, statename);	
			if(b){
				json.put("state", true);
			}else{
				json.put("state", false);
