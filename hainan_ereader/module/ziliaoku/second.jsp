...
<%--             <button class="button1" onclick="jump('<%=basePath %>ziliaoku/selectType.action');" type="button">分类维护</button>
 --%>        </div>
        ...
        <a style="font-size:18px;float:left;margin-left:50px;margin-top:8px;" href="<%=basePath %>ziliaoku/selectFirstType.action">资料库</a>
	       <span style="font-size:18px;float:left;margin-left:3px;margin-top:8px;">>></span>
	       <a style="font-size:18px;float:left;margin-top:8px;margin-left:0px;" href="<%=basePath %>ziliaoku/selectSecondType.action?typeid=${ftypeEntity.typeid}"><s:property value="ftypeEntity.name"/></a>
	       <div style="height:30px;"></div>
			<s:iterator value="secondList" status="st1">
				<s:if test="#st1.modulus(4)==1">
					<ul style="padding-top:10px;">
				</s:if>
				<a href="javascript:void(0)"
					onclick="jump('<%=basePath%>ziliaoku/select.action?typeid=${typeid}&pageNo=1');">
