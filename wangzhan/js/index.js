...
//    })
    ...
    RssApi.Table.List("releasum").condition({ "classifyid": 1 ,"state":1, "newsid":1}).keyvalue({ "pagesize": 1000 }).getJson(function (data) {
//    RssApi.Table.List("releasum").condition({ "classifyid": 1 ,"state":1, "newsid":1}).keyvalue({ }).getJson(function (data) {
        
        //data=  data.sort(function(a, b) { return a.shijian < b.shijian ? 1 : -1;} ); //按时间排序
        data.sort(compare("shijian"));
//
//       $(".nationalNews").each(function(){
...
//        });    
        ...
        localStorage.setItem("rendayaowenDataLength", dataLength);
        if (dataLength > 8) {
            dataLength = 8;
        }
        var liStr = "";
...
//        console.log("nationNewsLiStr is:", liStr);
    ...
    RssApi.Table.List("releasum").condition({ "classifyid": 1 ,"state":1, "newsid":2}).keyvalue({ "pagesize": 200 }).getJson(function (data) {
        data.sort(compare("shijian"));
        var liStr = "";
        var dataLength = data.length;
        if (dataLength > 8) {
...
//        console.log("nickname1:", data);
        ...
        $("#zhurenALink").attr("href", "zhuren_show.htm?id=" + data.id);
...
//        console.log("liStr is:", liStr);
    ...
    // 监督工作
    RssApi.Table.List("releasum").condition({ "classifyid": 6 ,"state":1}).keyvalue({ "pagesize": 12 }).getJson(function (data) {
...
//    RssApi.Table.List("releasum").condition({ "classifyid": 8 ,"state":1}).keyvalue({ "pagesize": 8 }).getJson(function (data) {
    RssApi.Table.List("imagin_news").condition({ "category": 1 ,"state":1}).keyvalue({ "pagesize": 8 }).getJson(function (data) {
        data.sort(compare("shijian"));
        var liStr = "";
        var liStr2 = "";
        for (var i = 0; i < data.length; i++) {
...
//                console.log(imgcont);
   ...
   RssApi.Table.List("imagin_news").keyvalue({"pagesize": 66 ,"category":2 }).getJson(function (_data) {
       ...
       if (data) {
           ...
           for (var i = 0; i < data.length; i++) {
                console.log("滚动图片新闻data.title:" + data[i].title);   
               var imgcont = data[i];
    ...
    let obj = {
     ...
     $(".lmTitle ul li a").mouseover((event)=>{
            ...
            if (id == 'rendayaowen') {
                ...
                $("#shizhengxinwen").removeClass("renDaBottom");
            } else if (id == 'shizhengxinwen') {
                $("#" + id).addClass("renDaBottom");
                $("#rendayaowen").removeClass("renDaBottom");
            } else {
        ...
        $("#shizhengxinwen").click(function(){
            location.href = "NewsCurrent.htm?classifyid=1&newsid=2";
        });
        ...
        $("#diaochayanjiu").click(function(){
            location.href = "investigation.htm?classifyid=15";
        });
